package com.example.slbappcomm.uploadimg.view;
 ...
 * 头像上传原图裁剪容器
 */
public class UploadImgClipViewLayout extends RelativeLayout {
    //裁剪原图
    private ImageView imageView;
    //裁剪框
    private UploadImgClipView uploadImgClipView;
    ...
    public UploadImgClipViewLayout(Context context) {
        this(context, null);
    }

    public UploadImgClipViewLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public UploadImgClipViewLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void init(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.UploadImgClipViewLayout);

        //获取剪切框距离左右的边距, 默认为50dp
        mHorizontalPadding = array.getDimensionPixelSize(R.styleable.UploadImgClipViewLayout_mHorizontalPadding,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, getResources().getDisplayMetrics()));
        //获取裁剪框边框宽度，默认1dp
        int clipBorderWidth = array.getDimensionPixelSize(R.styleable.UploadImgClipViewLayout_clipBorderWidth,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources().getDisplayMetrics()));
        //裁剪框类型(圆或者矩形)
        int clipType = array.getInt(R.styleable.UploadImgClipViewLayout_clipType, 1);

        //回收
        array.recycle();
        uploadImgClipView = new UploadImgClipView(context);
        //设置裁剪框类型
        uploadImgClipView.setClipType(clipType == 1 ? UploadImgClipView.ClipType.CIRCLE : UploadImgClipView.ClipType.RECTANGLE);
        //设置剪切框边框
        uploadImgClipView.setClipBorderWidth(clipBorderWidth);
        //设置剪切框水平间距
        uploadImgClipView.setmHorizontalPadding(mHorizontalPadding);
        ...
        this.addView(uploadImgClipView, lp);
    }
    ...
    public void initSrcPic(Uri uri) {
        ...
        String path = UploadImgGetFileUtil.getRealFilePathFromUri(getContext(), uri);
        ...
        if (bitmap.getWidth() >= bitmap.getHeight()) {//宽图
            ...
            Rect rect = uploadImgClipView.getClipRect();
            //高的最小缩放比
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mode == DRAG) { //拖动
                    ...
                    mVerticalPadding = uploadImgClipView.getClipRect().top;
