...
import com.example.slbappcomm.xclchart.common.IFormatterDoubleCallBack;
...
import com.example.slbappcomm.xclchart.renderer.plot.AxisTitle;
import com.example.slbappcomm.xclchart.renderer.plot.AxisTitleRender;
...
public class AxesChart extends EventChart {
    ...
    public AxesChart() {
        ...
        if (null == mLstDataTick) {
            mLstDataTick = new ArrayList<PlotAxisTick>();
        }
        if (null == mLstCateTick) {
            mLstCateTick = new ArrayList<PlotAxisTick>();
        }
        ...
        for (int i = 0; i < lstLabels.size(); i++) {
            ...
            switch (mCategoryAxisLocation) {
                ...
                case VERTICAL_CENTER:
                    if (!t.isShowTickMarks() || !isDrawYAxisTickMarks(t.Y, mMoveY)) {
                        showTicks = false;
                    }
    ...
    protected boolean isDrawYAxisTickMarks(float currentY, float moveY) {
        return Float.compare(currentY, plotArea.getTop() - moveY) != -1 &&
    ...
    protected boolean isDrawXAxisTickMarks(float currentX, float moveX) {
        if (Float.compare(currentX + moveX, plotArea.getLeft()) == -1) {
            return false;
        }
        return Float.compare(currentX + moveX, plotArea.getRight()) != 1;
    }
    ...
    protected float getAxisXPos(XEnum.AxisLocation location) {
        if (XEnum.AxisLocation.RIGHT == location) {    //显示在右边
