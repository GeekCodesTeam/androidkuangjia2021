...
import com.fosung.eduapi.presenter.EduResCompanyPresenter;
...
public class EduResCompanySelectExecutorListAct extends BaseActivity implements EduResCompanyView {
    ...
    private ArrayList<EduResCompanyExecutorReplyBean.DataBean> excludeExecutors;
    private ArrayList<EduResCompanyExecutorReplyBean.DataBean> resultList;
    private EduResCompanyPresenter eduResCompanyPresenter;

    private String searchString = "";
    ZRecyclerView zRecyclerView;
    SearchView searchView;
    BaseRecyclerAdapter<EduResCompanyExecutorReplyBean.DataBean> adapter;

    EduResCompanyExecutorReplyBean.DataBean orgBean;
    ArrayList<EduResCompanyExecutorReplyBean.DataBean> executors = new ArrayList<>();
    EduResCompanyExecutorReplyBean.DataBean currentExecutor;
    ...
    EduResCompanySelectedExecutorsUtil util;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        util = EduResCompanySelectedExecutorsUtil.getInstance(this);
...
//        .getDataList().size() + " 人/部门");
    ...
    private void configRecyclerView() {
        ...
        adapter = new BaseRecyclerAdapter<EduResCompanyExecutorReplyBean.DataBean>() {
            @Override
            ...
            public void setUpData(CommonHolder holder, int position, int viewType,
                ...
                checkedTextView.setText(data.orgName);
...
//                lowerLevelTextView.setEnabled(!data.checked);
                ...
                lowerLevelTextView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent intent = new Intent(EduResCompanySelectExecutorListAct.this,
                                EduResCompanySelectExecutorListAct.class);
...
//                        LHGLSelectExecutorListActivity.class,"bundle",bundle);
        ...
        eduResCompanyPresenter = new EduResCompanyPresenter();
        eduResCompanyPresenter.onCreate(this);
        eduResCompanyPresenter.getEduResCompanyRequest(currentExecutor.orgId);
    }


    @Override
    public void OnEduResCompanySuccess(EduResCompanyExecutorReplyBean resObj) {
        ArrayList<EduResCompanyExecutorReplyBean.DataBean> temList = new ArrayList<>();

        if (resObj.datalist != null && resObj.datalist.size() > 0) {
            for (EduResCompanyExecutorReplyBean.DataBean bean : resObj.datalist) {
    ...
    @Override
    public void OnEduResCompanyFail(String msg) {
        ToastUtil.toastShort(msg);
    }

    public void matchChecked(List<EduResCompanyExecutorReplyBean.DataBean> sourceList) {
//        int size = sourceList.size();
        int size = 0;
        for (EduResCompanyExecutorReplyBean.DataBean bean : sourceList) {
            if (bean.enable) {
                size++;
            }
        }
    ...
    private void configHorizontalScrollViewNavigation() {
        ...
        if (bundle != null) {
            ...
            ArrayList<EduResCompanyExecutorReplyBean.DataBean> superExecutors =
        ...
        orgTextView.setText(orgBean.orgName);
        ...
        for (int i = 0; i < executors.size(); i++) {
            ...
            EduResCompanyExecutorReplyBean.DataBean bean = executors.get(i);
            executorTextView.setText(bean.orgName);
            int finalI = i;
...
//    }
    ...
    public void onHeaderAllSelectCheckedTextViewClick(View view) {
        ...
        ArrayList<EduResCompanyExecutorReplyBean.DataBean> list = adapter.getDatas();
        for (int i = 0; i < list.size(); i++) {
            EduResCompanyExecutorReplyBean.DataBean bean = list.get(i);
            if (bean.enable) {
    ...
    public void onConfirmButtonClick(View view) {
        EventBus.getDefault().post(new EduResCommonEvent(EduResCommonEvent.BSDW));
//        setResult(RESULT_OK);
//        finish();
    ...
    private void getResult() {
        ...
        ArrayList<EduResCompanyExecutorReplyBean.DataBean> temSearchList = new ArrayList<>();
        for (EduResCompanyExecutorReplyBean.DataBean dataBean : resultList) {
            if (dataBean.orgName.contains(searchString)) {
