...
public final class MobLocalBroadcastManagers {
    ...
    private static volatile MobLocalBroadcastManagers mInstance;

    public static MobLocalBroadcastManagers getInstance(Context context) {
        Object var1 = mLock;
        synchronized (mLock) {
            if (mInstance == null) {
                mInstance = new MobLocalBroadcastManagers(context.getApplicationContext());
//                mInstance = new LocalBroadcastManagers(context);
            }
    ...
    private MobLocalBroadcastManagers(Context context) {
        this.mAppContext = context;
        this.mHandler = new Handler(context.getMainLooper()) {
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case 1:
                        MobLocalBroadcastManagers.this.executePendingBroadcasts();
                        break;
                    default:
    ...
    public void registerReceiver(BroadcastReceiver receiver, IntentFilter filter) {
        ...
        synchronized (this.mReceivers) {
            MobLocalBroadcastManagers.ReceiverRecord entry = new MobLocalBroadcastManagers.ReceiverRecord(filter, receiver);
                    ...
                    for (int j = 0; j < filter.countActions(); ++j) {
                        ...
                        if (receivers != null) {
                            for (int k = 0; k < receivers.size(); ++k) {
                                if (((MobLocalBroadcastManagers.ReceiverRecord) receivers.get(k)).receiver == receiver) {
                                    receivers.remove(k);
                                    --k;
                                }
                            }
        ...
        synchronized (this.mReceivers) {
            ...
            if (entries != null) {
                ...
                for (i = 0; i < entries.size(); ++i) {
                    MobLocalBroadcastManagers.ReceiverRecord receiver = (MobLocalBroadcastManagers.ReceiverRecord) entries.get(i);
                    if (debug) {
                    ...
                    if (receiver.broadcasting) {
                        ...
                        int match = receiver.filter.match(action, type, scheme, data, categories, "LocalBroadcastManagers");
                        if (match >= 0) {
                            if (debug) {
                ...
                if (receivers != null) {
                    for (i = 0; i < receivers.size(); ++i) {
                        ((MobLocalBroadcastManagers.ReceiverRecord) receivers.get(i)).broadcasting = false;
                    }

                    this.mPendingBroadcasts.add(new MobLocalBroadcastManagers.BroadcastRecord(intent, receivers));
    ...
    private void executePendingBroadcasts() {
        while (true) {
            MobLocalBroadcastManagers.BroadcastRecord[] brs = null;
            ...
            synchronized (this.mReceivers) {
                ...
                brs = new MobLocalBroadcastManagers.BroadcastRecord[br];
                this.mPendingBroadcasts.toArray(brs);
                this.mPendingBroadcasts.clear();
            }

            for (int var6 = 0; var6 < brs.length; ++var6) {
                MobLocalBroadcastManagers.BroadcastRecord var7 = brs[var6];
