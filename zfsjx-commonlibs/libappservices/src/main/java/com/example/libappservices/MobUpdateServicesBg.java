...
import com.geek.libutils.app.App;
...
//        startForeground(intent.getIntExtra(EXTRA_NOTIFICATION_ID, 1), she_notifichanged());
    ...
    public static Notification she_notifichanged() {
        NotificationManager notificationManager = (NotificationManager) App.get().getSystemService(NOTIFICATION_SERVICE);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            notification = new NotificationCompat.Builder(App.get(), UPDATE_CHANNEL_ID)
                    .setChannelId(UPDATE_CHANNEL_ID)
                    .build();
        } else {
            NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(App.get(), UPDATE_CHANNEL_ID)
                    .setOngoing(true)
