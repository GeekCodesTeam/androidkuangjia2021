...
/**
 * https://github.com/tenthbitinc/ZoomImageView
 */
public class ZoomImageView extends androidx.appcompat.widget.AppCompatImageView implements View.OnTouchListener,
    ...
    private class MultiGestureDetector extends GestureDetector.SimpleOnGestureListener implements
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if ((getDrawable() != null)
                    && ((scale < maxScale) || (scaleFactor < 1f) || (scale > 0.75))) {
        ...
        public boolean onDoubleTap(MotionEvent event) {
            try {
                ...
                if (scale < midScale) {
                    post(new AnimatedZoomRunnable(scale, getMidScale(), x, y));
     ...
     * depends on the current {@link ScaleType}
     */
    ...
    public float getMidScale() {
        ...
        float scale = 0;
        if (drawable != null) {
            float scaledHeight = drawable.getIntrinsicHeight();
            float scaledWidth = drawable.getIntrinsicWidth();
            if (scaledHeight < getMeasuredHeight()) {
                scale = getMeasuredHeight() / scaledHeight;
            } else if (scaledWidth < getMeasuredWidth()) {
                scale = getMeasuredWidth() / scaledWidth;
            }
        }
        return midScale > scale ? midScale : scale;
    }
