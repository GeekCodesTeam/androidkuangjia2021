...
package com.fosung.frameutils.http.okhttp.cookie.store;
...
public class PersistentCookieStore implements CookieStore {
    ...
    protected void add(HttpUrl uri, Cookie cookie) {
        ...
        if (cookie.persistent()) {
            if (!cookies.containsKey(uri.host())) {
                cookies.put(uri.host(), new ConcurrentHashMap<String, Cookie>(16));
            }
            cookies.get(uri.host())
                   .put(name, cookie);
        } else {
            if (cookies.containsKey(uri.host())) {
                cookies.get(uri.host())
        ...
        prefsWriter.putString(uri.host(), TextUtils.join(",", cookies.get(uri.host())
                                                                     .keySet()));
    ...
    public List<Cookie> get(HttpUrl uri) {
        ...
        if (cookies.containsKey(uri.host())) {
            Collection<Cookie> cookies = this.cookies.get(uri.host())
                                                     .values();
            for (Cookie cookie : cookies) {
    ...
    public boolean remove(HttpUrl uri, Cookie cookie) {
        ...
        if (cookies.containsKey(uri.host()) && cookies.get(uri.host())
                                                      .containsKey(name)) {
            cookies.get(uri.host())
                   .remove(name);
