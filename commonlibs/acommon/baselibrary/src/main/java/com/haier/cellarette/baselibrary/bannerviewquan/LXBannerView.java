...
public class LXBannerView<T> extends RelativeLayout {
    private static final String TAG = "LXBannerView";
    ...
    public LXBannerView(@NonNull Context context) {
        super(context);
        init();
    }

    public LXBannerView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        readAttrs(context, attrs);
        init();
    }

    public LXBannerView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    public LXBannerView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr, @StyleRes int defStyleRes) {
...
//                    setCurrent(getCurrent() - 1);
//                }
//                if (getViewPager().getOrientation() == HORIZONTAL) {
//                    mIsViewPager2Drag = distanceX > mTouchSlop && distanceX > distanceY;
//                } else {
...
//                }
    ...
    /**
     * 开始轮播
     * <p>应该确保在调用用了{@link LXBannerViewbeifen {@link #setPages(List, LXHolderCreator)}} 之后调用这个方法开始轮播</p>
     */
    public void start() {
     ...
     * @param mzHolderCreator ViewHolder生成器 {@link LXHolderCreator} And {@link LXViewHolder}
     */
     ...
     * 改变最小滑动距离
     */
    public LXBannerView setTouchSlop(int mTouchSlop) {
