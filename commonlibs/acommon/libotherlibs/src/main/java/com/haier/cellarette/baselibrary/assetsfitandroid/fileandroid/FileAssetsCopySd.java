    ...
    public static File copyResurces(String src, String dest, int flag) {
        ...
        try {
            if (flag == 0) {//copy to sdcard
                filesDir = new File(Utils.getApp().getExternalFilesDir(null).getAbsoluteFile() + "/shiningandgeek/" + dest);
                File parentDir = filesDir.getParentFile();
                if (!parentDir.exists()) {
                    parentDir.mkdirs();
                }
            } else {//copy to data
                filesDir = new File(BaselibApp.get().getFilesDir(), dest);
            }
            if (!filesDir.exists()) {
                filesDir.createNewFile();
                InputStream open = BaselibApp.get().getAssets().open(src);
