...
public class LiveGroupTIMUIController {
    ...
    public static void onDraw(ICustomMessageViewGroup parent, final LiveMessageInfo info, final String groupId) {
        ...
        View view = LayoutInflater.from(BaseApp.get()).inflate(R.layout.message_adapter_content_trtc, null, false);
        ...
        final String text = BaseApp.get().getString(R.string.no_support_msg);
        if (info == null) {

        } else {
            if (!TextUtils.isEmpty(info.anchorName)) {
                textLiveName.setText(info.anchorName + BaseApp.get().getString(R.string.live));
            } else {
                textLiveName.setText(info.roomName);
            }
            textStatus.setText(info.roomStatus == 1 ? BaseApp.get().getString(R.string.living) : BaseApp.get().getString(R.string.stop_live));
        }
    ...
    private static void createRoom(String groupId) {
        LiveRoomAnchorActivity.start(BaseApp.get(), groupId);
    }

    private static void enterRoom(LiveMessageInfo info) {
        Intent intent = new Intent(BaseApp.get(), LiveRoomAudienceActivity.class);
