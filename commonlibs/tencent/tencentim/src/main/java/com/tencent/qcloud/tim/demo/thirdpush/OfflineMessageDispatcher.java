...
public class OfflineMessageDispatcher {
    ...
    private static OfflineMessageBean offlineMessageBeanValidCheck(OfflineMessageBean bean) {
        if (bean == null) {
            ...
            String label = String.valueOf(packageManager.getApplicationLabel(BaseApp.get().getApplicationInfo()));
            ToastUtil.toastLongMessage(BaseApp.get().getString(R.string.you_app) + label + BaseApp.get().getString(R.string.low_version));
        ...
        if (bean.action == OfflineMessageBean.REDIRECT_ACTION_CHAT) {
            ...
            if (model != null) {
                ...
                if (timeout >= model.timeout) {
                    ToastUtil.toastLongMessage(BaseApp.get().getString(R.string.call_time_out));
                } else {
                    if (TextUtils.isEmpty(model.groupId)) {
                        ...
                        V2TIMManager.getSignalingManager().addInvitedSignaling(info, new V2TIMCallback() {
                            ...
                            public void onSuccess() {
                                ((TRTCAVCallImpl)(TRTCAVCallImpl.sharedInstance(BaseApp.get()))).
