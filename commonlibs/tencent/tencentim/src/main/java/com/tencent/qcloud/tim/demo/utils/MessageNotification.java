...
public class MessageNotification {
    ...
    private void createNotificationChannel(boolean isDialing) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (isDialing) {
                channel = new NotificationChannel(NOTIFICATION_CHANNEL_CALL, BaseApp.get().getString(R.string.call_notification), NotificationManager.IMPORTANCE_HIGH);
                channel.setSound(RingtoneManager.getDefaultUri(RingtoneManager.TYPE_RINGTONE), null);
                channel.setDescription(BaseApp.get().getString(R.string.background_tip));
                channel.setVibrationPattern(new long[]{0, 1000, 1000, 1000, 1000});
            } else {
                channel = new NotificationChannel(NOTIFICATION_CHANNEL_COMMON, BaseApp.get().getString(R.string.new_msg_notification), NotificationManager.IMPORTANCE_HIGH);
                channel.setDescription(BaseApp.get().getString(R.string.background_tip));
            }
    ...
    public void notify(V2TIMMessage msg) {
        ...
        String tickerStr = BaseApp.get().getString(R.string.new_msg);
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                ...
                mHandler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        builder.setContentText(BaseApp.get().getString(R.string.call_nos));
