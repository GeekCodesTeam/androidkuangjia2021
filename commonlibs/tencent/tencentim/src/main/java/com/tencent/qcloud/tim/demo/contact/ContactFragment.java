    ...
    private void initViews(View view) {
        ...
        mContactLayout.getContactListView().setOnItemClickListener(new ContactListView.OnItemClickListener() {
            ...
            public void onItemClick(int position, ContactItemBean contact) {
                if (position == 0) {
                    Intent intent = new Intent(BaseApp.get(), NewFriendActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    BaseApp.get().startActivity(intent);
                } else if (position == 1) {
                    Intent intent = new Intent(BaseApp.get(), GroupListActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    BaseApp.get().startActivity(intent);
                } else if (position == 2) {
                    Intent intent = new Intent(BaseApp.get(), BlackListActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    BaseApp.get().startActivity(intent);
                } else {
