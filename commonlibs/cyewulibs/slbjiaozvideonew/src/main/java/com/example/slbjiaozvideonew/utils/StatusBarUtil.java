...
//            statusBarHeight = context.getResources().getDimensionPixelSize(statusBarHeight);
//        } catch (Exception e1) {
//            e1.printStackTrace();
//        }
...
//    }
    ...
    public static void setStatusBar(Activity activity, int statusBar) {
        int statusBarHeight = StatusBarUtil.getStatusBarHeight(activity);
        View viewById = activity.findViewById(statusBar);
        if (viewById != null) {
     ...
     *
     * @param activity       fragment 对应的 activity_cert_img
     * @param needOffsetView 需要向下偏移的 View
     */
     ...
     */
    public static void setStatusViewAttr(View view, Activity activity, int colorMain) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            view.setBackgroundColor(colorMain);
        }
    }
}
