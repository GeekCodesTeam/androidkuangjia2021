...
//                vidSts.setRegion("cn-shanghai");
    ...
    public void release() {
        if (mMediaHandler != null && aliyunMediaPlayer != null) {
            final AliPlayer tmpMediaPlayer = aliyunMediaPlayer;
    ...
    public void onVideoSizeChanged(int i, int i1) {
        ...
        handler.post(new Runnable() {
            @Override
            public void run() {
                jzvd.onVideoSizeChanged(aliyunMediaPlayer.getVideoWidth(), aliyunMediaPlayer.getVideoHeight());
            }
        });
    }
    ...
    public void onError(final ErrorInfo errorInfo) {
        ...
        handler.post(new Runnable() {
            @Override
            public void run() {
                jzvd.onError(errorInfo.getCode().getValue(), errorInfo.getCode().getValue());
            }
        });
    }
    ...
    public void onInfo(final InfoBean infoBean) {
        if (infoBean.getCode() == InfoCode.AutoPlayStart) {
            ...
            //更新bufferedPosition
            final long videoBufferedPosition = infoBean.getExtraValue();
            handler.post(new Runnable() {
                @Override
                public void run() {
                    jzvd.setBufferProgress((int) videoBufferedPosition);
                }
            });
            ...
            handler.post(new Runnable() {
                @Override
                public void run() {
                    jzvd.onInfo(FROM_ALIYUN_PLAYER_INFO, infoBean.getCode().getValue());
                }
            });
        }
    }
