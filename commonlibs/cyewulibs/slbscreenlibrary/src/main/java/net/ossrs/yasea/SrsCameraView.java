...
public class SrsCameraView extends GLSurfaceView implements GLSurfaceView.Renderer {
    ...
    public int[] setPreviewResolution(int width, int height) {
        ...
        Camera.Size rc = adaptPictureResolution(mCamera.new Size(width, height));
        if (rs != null) {
            mPreviewWidth = rs.width;
            mPreviewHeight = rs.height;
        }
        if (rc != null) {
            mPictureWidth = rc.width;
            mPictureHeight = rc.height;
        }
        mCamera.getParameters().setPreviewSize(mPreviewWidth, mPreviewHeight);
        mCamera.getParameters().setPictureSize(mPictureWidth, mPictureHeight);
    ...
    public boolean startCamera() {
        ...
        int[] range = adaptFpsRange(Config.VFPS, params.getSupportedPreviewFpsRange());
    ...
    private Camera.Size adaptPreviewResolution(Camera.Size resolution) {
        ...
        for (Camera.Size size : mCamera.getParameters().getSupportedPreviewSizes()) {
            ...
            int w = size.width - resolution.height;
            int h = size.height - resolution.width;
            if (width == -1) {
                width = Math.abs(w);
                height = Math.abs(h);
            } else if (Math.abs(w) <= width && Math.abs(h) <= height) {
                width = Math.abs(w);
                height = Math.abs(h);
                best = size;
            }
    ...
    private Camera.Size adaptPictureResolution(Camera.Size resolution) {
        Camera.Size best = null;
        int width = -1;
        int height = -1;
