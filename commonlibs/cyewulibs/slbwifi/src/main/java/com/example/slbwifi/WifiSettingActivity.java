...
//        final Wifi saved = WifiDatabaseUtils.find(res.SSID);
        final Wifi saved = MmkvUtilsWifi.getInstance().get_common_json(res.SSID, Wifi.class);
        if (saved != null) {
            long startTime = SystemClock.uptimeMillis();
...
//        WifiDatabaseUtils.delete(ssid);
        MmkvUtilsWifi.getInstance().remove_common(ssid);
...
//                                return;
//                            }
                            if (MmkvUtilsWifi.getInstance().get_common_json(mLastConnectingWifi.getSsid(), Wifi.class) != null) {
                                return;
                            }
//                            WifiDatabaseUtils.save(mLastConnectingWifi);// 这个时候可以显示已连接
                            MmkvUtilsWifi.getInstance().set_common_json2(mLastConnectingWifi.getSsid(), mLastConnectingWifi);
                        }
                        lianjie_connet = "已连接";
                        mWifiAdaper.setLianjie_content(lianjie_connet);
                        updateWifiUI();
