...
public class LessPrimaryItemSampleFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_less_primary_itemelm, container, false);
    ...
    private void initLinkageData(LinkageRecyclerView linkage) {
        ...
        secondaryAdapterConfig.setViewBind(linkage);
    ...
    private static class LessLinkageSecondaryAdapterConfig implements
        ...
        public void setViewBind(LinkageRecyclerView linkage) {
        ...
        public void addItem(int position, ElemeGroupedItem item) {
            ...
            List<ElemeGroupedItem> items = linkage.getSecondaryAdapter().getItems();
            List<String> strings = linkage.getPrimaryAdapter().getStrings();
            if (item.isHeader) {
                items.add(position, item);
                strings.add(position, item.header);
                String clickedGroup = items.get(position).header;
                int index = strings.indexOf(clickedGroup);
                linkage.getHeaderPositions().add(index, position);
                linkage.getSecondaryAdapter().notifyItemInserted(position);
                linkage.getPrimaryAdapter().notifyItemInserted(index);
            } else {
                items.add(position, item);
                linkage.getSecondaryAdapter().notifyItemInserted(position);
            }
        ...
        public void removeItem(int position) {
            ElemeGroupedItem item = (ElemeGroupedItem) linkage.getSecondaryAdapter().getItems().get(position);
            if (item == null) {
                return;
            }
            List<ElemeGroupedItem> items = linkage.getSecondaryAdapter().getItems();
            ...
            if (item.isHeader) {
                ...
                linkage.getHeaderPositions().remove(index);
                linkage.getPrimaryAdapter().notifyItemRemoved(index);
            } else {
                items.remove(position);
                linkage.getSecondaryAdapter().notifyItemRemoved(position);
            }
        }
        ...
        public void updateItem(int position, ElemeGroupedItem item) {
            ...
            if (item.isHeader) {
                ...
                linkage.getSecondaryAdapter().notifyItemChanged(position);
