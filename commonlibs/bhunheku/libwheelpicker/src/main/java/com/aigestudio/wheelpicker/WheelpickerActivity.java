...
public class WheelpickerActivity extends AppCompatActivity implements View.OnClickListener/*, SUpdateUserInfoView, SGetUserInfoView, SConfigureView*/ {

    private AutoHideInputMethodFrameLayoutlibwheelpicker afl1;
...
//    private SConfigurePresenter sConfigurePresenter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wheelpickerdemo);
        findview();
        onclick();
        donetwork();
        //
...
//            MobclickAgent.onEvent(this, "babyinfo_ok_button");
    ...
    private boolean is_yanzheng() {
        if (TextUtils.isEmpty(edt1.getText().toString().trim())) {
            Toast.makeText(WheelpickerActivity.this, "请输入宝宝小名", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (TextUtils.isEmpty(tv_birthday2.getText().toString().trim())) {
            Toast.makeText(WheelpickerActivity.this, "请选择出生日期", Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
