...
public class SrsEncoder {
     ...
     * @param mp
     */
    public SrsEncoder(SrsEncodeHandler handler, MediaProjection mp) {
        mHandler = handler;
        mMediaProjection = mp;
        mVideoColorFormat = chooseVideoEncoder();
    }
    ...
    public boolean start() {
        ...
        if (mMediaProjection != null) {
            ...
            videoFormat.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, Config.KEY_I_FRAME_INTERVAL);
        } else {
            videoFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT, mVideoColorFormat);
            videoFormat.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, VGOP / Config.VFPS);
        }
//        videoFormat.setInteger(MediaFormat.KEY_PROFILE, MediaCodecInfo.CodecProfileLevel.AVCProfileBaseline);
//        videoFormat.setInteger("level", 0x01);
        videoFormat.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, 0);
        videoFormat.setInteger(MediaFormat.KEY_BIT_RATE, vBitrate);
        videoFormat.setInteger(MediaFormat.KEY_FRAME_RATE, Config.VFPS);
//        videoFormat.setInteger(MediaFormat.KEY_CAPTURE_RATE, Config.VFPS);
//        videoFormat.setInteger(MediaFormat.KEY_CHANNEL_COUNT, 1);
        videoFormat.setInteger(MediaFormat.KEY_REPEAT_PREVIOUS_FRAME_AFTER, 10000);

        vencoder.configure(videoFormat, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE);
        if (mMediaProjection != null) {
            mSurface = vencoder.createInputSurface();
        }
        ...
        if (mMediaProjection != null) {
            Thread sworker = new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        mVirtualDisplay = mMediaProjection.createVirtualDisplay(TAG + "-display",
                                vOutWidth, vOutHeight, 1, DisplayManager.VIRTUAL_DISPLAY_FLAG_OWN_CONTENT_ONLY,
                                mSurface, null, null);
                        onProgress = true;
                        while (onProgress) {
                            onProcessedYuvFrame();
    ...
    public void setVideoHDMode() {
        vBitrate = Config.vHDBitrate;  // 1200 kbps
        x264Preset = "veryfast";
    }
    ...
    public void setVideoSmoothMode() {
        vBitrate = Config.vSmoothBitrate;  // 500 kbps
     ...
     * 屏幕硬编码
     */
    public void onProcessedYuvFrame() {
        int outBufferIndex = vencoder.dequeueOutputBuffer(vebi, 10000);
        if (outBufferIndex >= 0) {
            System.out.println(outBufferIndex);
            ByteBuffer realData = vencoder.getOutputBuffer(outBufferIndex);
            vebi.presentationTimeUs = System.nanoTime() / 1000 - mPresentTimeUs;
            flvMuxer.writeSampleData(videoFlvTrack, realData, vebi);
            vencoder.releaseOutputBuffer(outBufferIndex, false);
        }
    }
    ...
    private MediaCodecInfo chooseVideoEncoder(String name) {
        ...
        for (int i = 0; i < codecInfos.length; i++) {
            MediaCodecInfo mci = codecInfos[i];
            if (!mci.isEncoder()) {
                continue;
            }
