...
public class SrsFlvMuxer {
    ...
    private LinkedBlockingQueue<SrsFlvFrame> mFlvTagCache = new LinkedBlockingQueue<>(200);
    ...
    private void sendFlvTag(SrsFlvFrame frame) {
        if (!publisher.isConnected() && started) {
            // 重连
            long currentTimeMillis = System.currentTimeMillis();
            if (SimplePublisher.getInstance().isRecording() && (currentTimeMillis - time) > 1000 && started) {
                time = currentTimeMillis;
                publisher.close();
                connect(url);

                if (publisher.isConnected() && (mVideoSequenceHeader != null || mAudioSequenceHeader != null) && started) {
                    if (mVideoSequenceHeader != null) {
                        sendFlvTag(mVideoSequenceHeader);
                    }
                    if (mAudioSequenceHeader != null) {
                        sendFlvTag(mAudioSequenceHeader);
                    }
                    sendMetaData(0, false);
                }
            }
            return;
        }
        ...
        worker = new Thread(new Runnable() {
            ...
            public void run() {
                while (!Thread.interrupted() && started) {
                    while (!mFlvTagCache.isEmpty() && started) {
...
//            }
    ...
    public void sendMetaData(int orientation, boolean isStop) {
        if (publisher.isConnected()) {
            publisher.onMetaData(orientation, isStop);
        }
    }
