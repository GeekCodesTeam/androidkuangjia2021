...
//        startForeground(HUYAN_MANAGE_NOTIFICATION_ID, new Notification());
    ...
    public void onCreate() {
        super.onCreate();
        MyLogUtil.e("BleService", "onCreate");
//        bleManager = BleManager.getInstance();
    ...
    private void startLeScan() {
        if (mBluetoothAdapter == null) {
            BluetoothManager bluetoothManager = (BluetoothManager) BaseApp.get().getSystemService(BLUETOOTH_SERVICE);//这里与标准蓝牙略有不同
            mBluetoothAdapter = bluetoothManager.getAdapter();
        }
        if (mBluetoothAdapter == null || !mBluetoothAdapter.isEnabled()) {
            MyLogUtil.e(TAG, "蓝牙没有连接");
            return;
        }
...
//                    Thread.sleep(500);
                    ...
                    MyLogUtil.e(TAG, "bleDataHandler 报错：" + e);
                }
            }
        };
//        }
...
//            MyLogUtil.e(TAG, deviceName + deviceAddr + broadcastPack2 + broadcastPack);
//            MyLogUtil.E(TAG, "deviceNumList:" + deviceNumList);
//            if (bleDeviceMatchCallbackSet.size() == 0)
//                return;
//            MyLogUtil.e(TAG, "KKKKKKKKKKKKKKKKKKKKKKKKKK");
    ...
    public void stateChange(int blueState) {
        switch (blueState) {
            case BluetoothAdapter.STATE_OFF:
                MyLogUtil.e(TAG, "蓝牙关闭");
                break;
            case BluetoothAdapter.STATE_TURNING_OFF:
                MyLogUtil.e(TAG, "蓝牙正在关闭");
                break;
            case BluetoothAdapter.STATE_ON:
                MyLogUtil.e(TAG, "蓝牙开启");
                startLeScan();
                break;
            case BluetoothAdapter.STATE_TURNING_ON:
                MyLogUtil.e(TAG, "蓝牙正在开启");
                break;
        }
    }
...
//        MyLogUtil.e(TAG, "测试测试测试测试测试测试测试测试测试测试");
        //
...
//                MyLogUtil.E(TAG, member.toString());
