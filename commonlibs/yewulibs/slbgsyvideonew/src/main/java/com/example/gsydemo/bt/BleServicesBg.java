...
//        startForeground(intent.getIntExtra(EXTRA_NOTIFICATION_ID, 1), she_notifichanged());
    ...
    public static Notification she_notifichanged() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            notification = new NotificationCompat.Builder(BaseApp.get(), Bt_CHANNEL_ID)
                    .setChannelId(Bt_CHANNEL_ID)
                    .build();
        } else {
            NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(BaseApp.get(), Bt_CHANNEL_ID)
                    .setOngoing(true)
