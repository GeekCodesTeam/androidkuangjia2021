...
//            String newFileDir = getExternalFilesDir(null) + OTASendUtils.DIR_DOWNLOAD;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null && !intent.getStringExtra("romurl").isEmpty() && !intent.getStringExtra("rommd5").isEmpty()) {
            ...
            MyLogUtil.d(TAG, "start download rom_url =" + romurl);
            MyLogUtil.d(TAG, "download  rom md5  rommd5 =" + rommd5);
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (file.exists()) {
                        file.delete();
                        MyLogUtil.d(TAG, "del rom success");
                    }
                    //开始执行下载任务
                    FileDownloader.getImpl().create(romurl).setPath(llsRomFilePath).setListener(queueTarget).start();
                }
            ...
            MyLogUtil.d(TAG, "download rom_url  is null");
            stopSelf();
        }
...
//        MsgViewManager.destory();
        ...
        MyLogUtil.d(TAG, " ROM download save path path =" + saveFile.toString());
        MyLogUtil.d(TAG, " ROM download save path path =" + saveFile.getAbsolutePath());
        String strRomMD5 = "";
        try {
            strRomMD5 = Md5Utils.getFileMD5String(saveFile);
            MyLogUtil.d("strRomMD5:", strRomMD5);
            MyLogUtil.d("rommd5_:", rommd5);
        } catch (IOException e) {
            e.printStackTrace();
        }
        if (strRomMD5.equals(rommd5)) {
            MyLogUtil.d(TAG, "ROM verify md5 success");
            if (!Build.DEVICE.equals(OTASendUtils.TAG_FIRST_MAINBOARD)) {//BOE ROM升级
                ...
                handler.postDelayed(new Runnable() {
                    public void run() {
                        ...
                        sendBroadcast(intent);
                        MyLogUtil.d(TAG, "sTAG_SECOND_MAINBOARD");
                        MyLogUtil.d(TAG, "send upgrade ROM broadcast success");
                        MyLogUtil.d(TAG, "start upgrade ROM now");
                    }
                }, 20000);
...
//                MyLog.d(TAG, "start install ROM now");
            ...
            MyLogUtil.d(TAG, " ROM verify md5 error");
        }
    }
