...
public abstract class DraggableDrawer extends PLVMenuDrawer {
    ...
    protected void initDrawer(Context context, AttributeSet attrs, int defStyle) {
        ...
        mScroller = new Scroller(context, PLVMenuDrawer.SMOOTH_INTERPOLATOR);
...
//            setDrawerState(STATE_OPENING);
            setDrawerState(getPosition() == Position.BOTTOM ? STATE_CLOSING : STATE_OPENING);//polyv
            mScroller.startScroll(startX, 0, dx, 0, duration);
        } else {
//            setDrawerState(STATE_CLOSING);
            setDrawerState(getPosition() == Position.BOTTOM ? STATE_OPENING : STATE_CLOSING);//polyv
            mScroller.startScroll(startX, 0, dx, 0, duration);
        }
    ...
    protected boolean canChildScrollVertically(View v, boolean checkV, int dx, int x, int y) {
        if (v instanceof ViewGroup) {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                final int childRight = child.getRight() + supportGetTranslationX(child);
                final int childTop = child.getTop() + supportGetTranslationY(child);
                int childBottom = child.getBottom() + supportGetTranslationY(child);
                if (childBottom < y && getPosition() == Position.BOTTOM) {
                    childBottom = y + 1;//polyv
                }
                if (x >= childLeft && x < childRight && y >= childTop && y < childBottom
