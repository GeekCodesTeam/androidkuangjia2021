...
public class LiveGroupTIMUIController {
    ...
    public static void onDraw(ICustomMessageViewGroup parent, final LiveMessageInfo info, final String groupId) {
        ...
        View view = LayoutInflater.from(TencentApp.get()).inflate(R.layout.message_adapter_content_trtc, null, false);
        ...
        final String text = TencentApp.get().getString(R.string.no_support_msg);
        if (info == null) {

        } else {
            if (!TextUtils.isEmpty(info.anchorName)) {
                textLiveName.setText(info.anchorName + TencentApp.get().getString(R.string.live));
            } else {
                textLiveName.setText(info.roomName);
            }
            textStatus.setText(info.roomStatus == 1 ? TencentApp.get().getString(R.string.living) : TencentApp.get().getString(R.string.stop_live));
        }
    ...
    private static void createRoom(String groupId) {
        LiveRoomAnchorActivity.start(TencentApp.get(), groupId);
    }

    private static void enterRoom(LiveMessageInfo info) {
        Intent intent = new Intent(TencentApp.get(), LiveRoomAudienceActivity.class);
