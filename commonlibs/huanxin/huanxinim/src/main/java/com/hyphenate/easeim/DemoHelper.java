...
public class DemoHelper {
    ...
    private void InitCallKit(Context context){
        ...
        //设置声网AgoraAppId
        callKitConfig.setAgoraAppId("35fad4d0d34141079356dc243dfdc634");
    ...
    public void initPush(Context context) {
        if(EaseIM.getInstance().isMainProcess(context)) {
            ...
            //HMSPushHelper.getInstance().initHMSAgent(BaseApp.get());
    ...
    public void logoutSuccess() {
        Log.d(TAG, "logout: onSuccess");
        setAutoLogin(false);
        DemoDbHelper.getInstance(BaseApp.get()).closeDb();
    }
    ...
    public DemoModel getModel(){
        if(demoModel == null) {
            demoModel = new DemoModel(BaseApp.get());
        }
        return demoModel;
    }
    ...
    public synchronized int deleteContact(String username) {
        ...
        DemoDbHelper helper = DemoDbHelper.getInstance(BaseApp.get());
