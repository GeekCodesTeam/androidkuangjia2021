...
import com.hyphenate.easeim.IMMainActivity;
...
//            if(!BaseApp.get().getLifecycleCallbacks().isFront()){
    ...
    private synchronized boolean isAppLaunchMain() {
        ...
        if (activities != null && !activities.isEmpty()) {
            for (int i = activities.size() - 1; i >= 0; i--) {
                if (activities.get(i) instanceof IMMainActivity) {
                    return true;
                }
            }
        }
    ...
    private class ChatMultiDeviceListener implements EMMultiDeviceListener {
        ...
        public void onContactEvent(int event, String target, String ext) {
            ...
            DemoDbHelper dbHelper = DemoDbHelper.getInstance(BaseApp.get());
            String message = null;
            switch (event) {
