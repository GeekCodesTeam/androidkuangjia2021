...
public class SdcardUtil {
	...
	if (checkCard() == 0) {
	    StatFs sf = new StatFs(Utils.getApp().getExternalFilesDir(null).getAbsolutePath());
	...
	String directoryName = Utils.getApp().getExternalFilesDir(null).toString() + "/DCIM";
	...
	if (hasStorageCard()) {
	    getSizeInfo(Utils.getApp().getExternalFilesDir(null), info);
	} else {
	    info.availdSize = 0;
	    info.totalSize = 0;
	}
