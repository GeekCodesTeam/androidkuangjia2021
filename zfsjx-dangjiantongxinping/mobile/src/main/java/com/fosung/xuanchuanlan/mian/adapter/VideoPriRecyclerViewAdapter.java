...
public class VideoPriRecyclerViewAdapter extends RecyclerView.Adapter<VideoPriRecyclerViewAdapter.ViewHolder> {
    //数据源
    private List<VideoDetailBean.DataBean.PRIVATEBean> mList;

    public VideoPriRecyclerViewAdapter(List<VideoDetailBean.DataBean.PRIVATEBean> list) {
        mList = list;
    }

    public void resetSel(int pos) {
        if (mList == null) return;
        for (VideoDetailBean.DataBean.PRIVATEBean p : mList) {
            p.isPlay = false;
        }
        if (pos == -1) return;
    ...
    public void setdata(List<VideoDetailBean.DataBean.PRIVATEBean> list) {
        mList = list;
    }
