...
import com.example.slbappcomm.xclchart.event.click.PointPosition;
...
import com.example.slbappcomm.xclchart.renderer.line.PlotCustomLine;
    ...
    protected void renderBezierCurveLine(Canvas canvas, Paint paint,
        ...
        if (count == 3) {
            ...
            PointF ctl3 = PointHelper.percent(lstPoints.get(1), 0.5f,
                    lstPoints.get(2), 0.8f);
            bezierPath.quadTo(ctl3.x, ctl3.y, lstPoints.get(2).x,
                    lstPoints.get(2).y);
...
//        drawGradient(paint, start, stop, startColor, stopColor, colors);
        canvas.drawPath(bezierPath, paint);
        bezierPath.reset();
    }

    private void drawGradient(Paint paint, PointF start, PointF stop, int startColor, int stopColor, int[] colors) {
        if (startColor == 0 && stopColor == 0) {
            return;
        }
        LinearGradient linearGradient = new LinearGradient(start.x, start.y, stop.x, stop.y, colors, null, Shader.TileMode.MIRROR);
